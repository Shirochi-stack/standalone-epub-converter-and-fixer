...
- **Fix EPUB**: Select one or more existing `.epub` files. The app extracts each EPUB, runs it through the chapter extraction process, then recompiles it. This is useful for repairing EPUB structure, navigation, metadata, and compatibility issues.
...
Runtime settings are saved to `config.json` next to the script, or next to the executable in frozen builds. Generated builds, caches, local config, and temporary outputs are intentionally ignored by Git.
